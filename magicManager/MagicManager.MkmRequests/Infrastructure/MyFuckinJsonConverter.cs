...
namespace MagicManager.MkmRequests.Infrastructure
{
            ...
            while (reader.Read())
            {
                ...
                if (reader.Value != null)
                {
                    ...
                    if (MyObject.GetType().GetProperties().Where(p => p.Name == reader.Value.ToString()).Count() > 0)
                    {
                        ...
                        {
                            //on fait appel au tout premier constructeur de cette classe, à savoir celui qui a le moins d'arguments.
                            PropClass = pi.PropertyType.GetConstructors()[0].Invoke(null);

                        }
                    }
                    //Si ce n'est pas une propriété c'est une valeur, que nous allons affecter à "PI" (prop info)
                    else
                    {
                        ...
                        if (PropClass == null)
                        {
                            ...
                            //est ce que c'est un int64? Si oui on le convertit en int32
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                for (int i = 0; i < piObj.Length; i++)
                                {
                                    ...
                                    //Si le reader contient quelque chose
                                    if (reader.Value != null)
                                    {
